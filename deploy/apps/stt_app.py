...
class SpeechToTextApp(HydraHeadApp):
    ...
    def generate_sidebar(self):
        with st.sidebar:            
            st.session_state.acoustic_model_option = st.selectbox("Which acoustic model?", 
                                                                ('Model 50k', 'Model 130k'))
            st.info(f"You selected:{st.session_state.acoustic_model_option}")
            
            st.session_state.lm_option = st.selectbox("Which language model?", 
                                                    ('CTC', 'CTC + 4-gram'))
            st.info(f"You selected:{st.session_state.lm_option}")
        
        if st.session_state.acoustic_model_option == "" or st.session_state.lm_option == "":
            st.sidebar.warning("Setting must not be empty")
        
        st.session_state.predict_arg = {"model": st.session_state.acoustic_model_option,

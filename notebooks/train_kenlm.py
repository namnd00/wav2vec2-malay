...
def optimize(lm_dir, wav2vec_model_path, root_dir, path_data):
    global processor
    global model
    global vocab
    global wer
    ...
    test_dataset = Dataset.form_pandas(df)
    wer = load_metric("wer")
...
def main(lm_root_dir, wav2vec2_model_path,root_dir, path_data, **args):
    #lm_file_path=train_kenlm(lm_root_dir, "unshuffled_deduplicated_cy")
    lm_file_path = lm_root_dir
    optimize(lm_file_path, wav2vec2_model_path, root_dir, path_data) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", dest="wav2vec2_model_path", required=True, help="acoustic model to be used for optimizing")
    parser.add_argument("--root_dir", default = "/content/sample_data/datasets/waves/", required=False, help="target directory for language model")
    parser.add_argument("--path_data", default = "/content/sample_data/datasets/eval.csv", required= False, help="acoustic model to be used for optimizing")       
